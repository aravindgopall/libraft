...
defaultRaftNodeMetrics = RaftNodeMetrics
  ...
  , lastLogHashLabel = toS $ Char8.unpack (unEntryHash genesisHash)
  , lastLogIndexGauge = 0
  , commitIndexGauge = 0
  }
...
setLastLogEntryHashLabel
  ...
  => LastLogEntry v
  -> m ()
setLastLogEntryHashLabel = setRaftNodeLabel LastLogEntryHashLabel . toS . Char8.unpack . unEntryHash . hashLastLogEntry
...
lookupLastLogEntryHashLabel sample =
    case HashMap.lookup (show LastLogEntryHashLabel) sample of
      ...
      _ -> toS . Char8.unpack . unEntryHash $ genesisHash
